...
|---|---|---|
| Docker trademark | launch-blocking | No "docker" or whale in name/logo/package IDs (hence Skrog); nominative references only; ship CLI binaries unmodified under Apache-2.0; engine binaries built from source in CI, not repackaged from Docker's distribution channels. Legal sanity check before the repo goes public. **Amended 2026-09-21 (#450):** one CLI binary is now *built* rather than shipped unmodified — the Windows arm64 `docker.exe`, because upstream publishes none and `skrog cli install` on ARM was skipping the one command anybody types. Building from Apache-2.0 source in our own CI is the *engine* treatment, which this same line already sanctions, and it is the opposite of the repackaging the trademark concern is about: nothing is redistributed from Docker's channels, and the binary carries no Docker build string (`PlatformName` is deliberately left unset). amd64 stays Docker's own published bytes. The asymmetry is documented in `docs/docker-cli.md` and ends when upstream ships an arm64 build. |
