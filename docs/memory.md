...
```
VM        4 CPUs, CPU 9.6%   memory 575.2 MiB used of 7.6 GiB (7.1 GiB available)
          used is 95.9 MiB processes, 156.9 MiB page cache, 51.3 MiB kernel
          stalled (PSI, last 10s): cpu 0.0%  memory 0.0%  io 0.0%
Windows   vmmem 431.5 MiB (Task Manager's figure), working set 436.8 MiB, committed 954.5 MiB

                                     MEMORY     ANON       FILE       KERNEL     CPU%  MEM PSI  IO PSI
skrog-t-wrong                        15.2 MiB   14.4 MiB   0 B        400.0 KiB  0.0   0.0      0.0
skrog-t-any                          14.8 MiB   14.4 MiB   16.0 KiB   416.0 KiB  0.0   0.0      0.0
engine (dockerd, containerd, build)  192.8 MiB  66.0 MiB   118.6 MiB  7.6 MiB    7.4   0.0      0.0
other WSL distros (0)                0 B        0 B        0 B        0 B        0.0   0.0      0.0
WSL itself                           616.0 KiB  100.0 KiB  0 B        92.0 KiB   0.4   0.0      0.0
not charged to any group (kernel)    339.5 MiB
```
...
to back. It splits into process memory, **page cache** (file data kept in
memory: image layers, build contexts, anything read or written recently) and
the kernel's own structures.
...
| not charged to any group | used memory no group accounts for — mostly the kernel. **Derived**, not measured: used minus every top-level group |
...
The thresholds are a judgement about where it stops being noise, not a
measurement.
